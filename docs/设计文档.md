...
可用的 `outbound_tag` 值：`direct`、`blocked`，以及配置了 `[landing_proxy]` 时的 `landing-proxy`。不需要也不允许引用订阅节点 tag、`api` 或 balancer；API 规则和 balancer 兜底规则均由 kraymini 自动生成。
...
- `[[routing.rules]]` 中必须指定 `outbound_tag`，且其值必须是 `direct`、`blocked` 或 `landing-proxy`（仅当配置了 `[landing_proxy]` 时）之一
...
`selector` 包含所有订阅节点的 tag，负载均衡策略固定为 `leastPing`，与 observatory 配合实现自动选优。
...
| `outbounds` | 由订阅节点 + `[landing_proxy]` + 固定的 `direct` / `blocked` 生成 |
...
| `routing.balancers` | 由订阅节点列表生成，strategy 固定为 `leastPing` |
