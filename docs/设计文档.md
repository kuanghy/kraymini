...
| `file` | string | 否 | `""` | 日志文件路径，为空则输出到 stderr；kraymini 与 xray 共用此文件 |
...
xray 配置中不设置 `log.access` 和 `log.error` 文件路径，使 xray 将日志统一输出到标准流（stderr）。kraymini 通过 `Popen` 管理 xray 的日志输出：

- `[log].file` 非空时：kraymini 打开该文件，将 xray 子进程的 stderr 重定向到此文件；kraymini 自身也写入同一文件
- `[log].file` 为空时：xray 继承 kraymini 的 stderr，两者日志均输出到终端
